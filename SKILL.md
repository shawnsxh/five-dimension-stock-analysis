...
description: "基于博主Art of Speculation的五维分析框架（基本面+技术面+消息面+情绪面+期权结构）对个股和大盘进行多维度共振分析。当用户要求分析某只股票、判断大盘方向、或做交易决策前的多维交叉验证时触发，无需用户使用特定关键词。输出结构化分析报告。适用于美股、A股、港股。"
---
...
在聊天框中输出简要总结，**不是**完整报告。按逻辑线自然展开：
- 公司一句话定位
- 共振评分表（五维方向 + 置信度）
- 整体方向 + 建议仓位
- 核心判断——先讲多数维度一致的看多/看空理由，再指出矛盾点（哪个维度信号不一致），然后按五维冲突规则给出判断结论（以高置信度维度为准），最后落脚关键价位和下一催化剂
...
分析市场和个股是在不确定性里找到概率更高的方向，然后用仓位管理控制风险。